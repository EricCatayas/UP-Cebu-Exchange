...
const RentalOrderCard = ({ order, children }: { order: RentalOrderDTO; children?: React.ReactNode }) => {
  ...
  return (
    <div className="rounded-xl border border-gray-200 shadow-sm">
      ...
      {/* Content */}
      <dl className="text-sm space-y-2 px-4 bg-white pb-4">
        {order.user && (
          <div className="flex justify-between">
            <dt className="text-gray-500">From</dt>
            <dd className="font-medium text-gray-900">{order.user.fullName}</dd>
          </div>
        )}
        <div className="flex justify-between">
          <dt className="text-gray-500">Order Date</dt>
          <dd className="font-medium text-gray-900">{fmtDate(order.createdAt)}</dd>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <dd className="font-medium text-gray-900">{order.durationMonths} months</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-gray-500">Remaining</dt>
          <dd className="font-medium text-gray-900">{daysLeft} days</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-gray-500">Status</dt>
          <dd className="font-medium text-gray-900">{orderStatus.label}</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-gray-500">Payment Method</dt>
          <dd className="font-medium text-gray-900">{order.payment.method}</dd>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <dd className="font-medium text-gray-900">₱{order.payment.amount.toLocaleString('en-PH')}</dd>
        </div>
      </dl>
      {/* Links */}
